...
- [ ] **(in progress — branch `chore/trim-dead-move-examples`, NOT merged, eval gate RED)** **Shrink the base prompt; move its rules onto the tools.** `_BASE` is now identity + the non-negotiable contract; the move-translation block, judgment-question routing, and the resign/new_game confirm dance moved onto `make_move` / `evaluate_position` / `analyze_last_move` / `new_game` / `resign`, where a rule sits with the capability it governs. Chess flavor condensed to three sentences. System prompt 1,521 → 755 tok; narrate (offered no tools) sheds the full 766. Tests migrated (prompt-content → tool-description assertions in `test_tools.py`); golden updated. **Known regression: `long_capture` dropped `live_like` 4→3/5 and `poisoned` 4→0/5** — in noisy contexts the model stops recognizing a descriptive move ("grab the pawn on e6") as a call to `make_move` (all poisoned runs made zero tool calls). Pulling the move-*recognition* signal out of the prompt's primacy slot did it. **Deliberately not patched in the prompt** — this is coupled to the tools-return-info / context-injection slice below, which should cure it by cleaning the context the recognition drowns in. **The two merge together; neither ships alone.**
- [ ] **[S] Token + call-count observability in traces** (audit finding 1) — **do this first.** `ChatResult.usage` is parsed then dropped and `turn_record` counts nothing, so every cut in this sprint is currently invisible. Instrument the *provider boundary* (one collector every model call passes through — catches narrate too) and record per-call prompt/completion tokens + model-call count in the JSONL. No behavior change; after it lands, every slice below arrives with a measured before/after.
- [ ] **[S] Clean the tool-definition schema noise** (audit finding 5) — ~216 tok of pydantic `title` keys, `anyOf:[X,null]`, and `default:null` that teach the model nothing, plus a few duplicated docstring lines. One helper in `_derive_schema`, guarded by the golden test. Mechanical, zero semantic change — and worth doing now that we've just bulked this layer up with relocated rules.
- [ ] **[M] Tools return what the agent needs; turn-summaries replace the raw window** (audit findings 2/4/7 — the sprint's real theme). Make each tool result carry the fact the agent would otherwise have to remember or re-fetch (an illegal `make_move` returns the legal moves; the confirm-gate refusal already carries the exact line to relay — extend that pattern across the surface). Then reconsider the 20-turn transcript window: a per-turn summary the pipeline writes may hold banter continuity for a fraction of the tokens. **Design before code** — this reshapes what one brain turn even contains.
