...
- [ ] **[S] Read the repeat-stop narrator's token counts (needs an idle GPU — the instrument now exists).** Both instruments are built and neither has been read: the latency half settled *where* the 2–3× goes (the single narrator call, exact permutation p = 0.00088 — `docs/agent-evals.md`) and the token half (2026-07-27) settles *why*, because "emitted more reasoning tokens" and "generated slower" are the same milliseconds. The arm is not free: repeats are the ~10–25% tail, so re-run **`hints_off_no_advice` alone at `CHESSAPP_EVAL_RUNS=20 CHESSAPP_EVAL_MAX_RUNS=20`** with `CHESSAPP_EVAL_REPORT` set and nothing else on the card, then compare the `no_progress` and `completed` samples on `narrator_out` and `narrator_tok_s` the way the recorded table compares `narrator_ms`. **Then, and only then, decide the standing candidate fix** (bounding the narrator's thinking budget, deferred three times): a flat rate with 3× the tokens says the cap is aimed right; flat tokens at a third of the rate says the serving path is the finding and a cap would not touch it. That fix *is* a loop change with a real eval gate behind it, which is why the number it would be set to must come from a measurement rather than a guess.
- [ ] **[S] Does the planner flip hints on when nobody asked? — the instrument is built; read it.** (Observation, filed by the repeat-stop slice — *still not verified*.) Across the four `hints_off_no_advice` runs on 2026-07-26 it appears at a steady **9/65 samples** (3/20, 3/20, 2/20, 1/5): a `set_hints_mode` call on a turn where the player only asked "what should I play here?". If it turned hints **on**, that is a setting the player controls being changed by an agent that was asked a question, which no gate covers today (settings tools are ungated by design and the honesty guard checks settings *claims*, not unasked changes). Step one landed 2026-07-27 — the trajectory now prints arguments (`set_hints_mode(enabled=true)`) on the `[eval]` line **and in every per-sample report record**, so the count is a query over the report rather than a hand tally across scrollback. What remains is to **read it and decide**: this arm is the same 20-sample `hints_off_no_advice` run the repeat-stop token item needs, so run it once with `CHESSAPP_EVAL_REPORT` set and answer both questions off the one report. If the flip is real, the fix is a capability question, not a prompt line: the scenario's `check` should assert `app.ctx.settings.hints_mode is False` after the turn.
