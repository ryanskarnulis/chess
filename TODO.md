...
- [ ] **[S] A repeat-stop turn narrates 2–3× slower than an ordinary one** (spin-off, measured by the repeat-stop slice — cause **not** established). On the two 20-sample `hints_off_no_advice` arms, `no_progress` turns took a median 40.5 s (no planner note) and ≈26.7 s (with the loop's note) against 9.9–11.5 s for `completed` samples in the same runs, at the same model-call count. Two readings the old harness could not tell apart: the narrator's own round trip is slow, or the repeat and the long narration share a cause (the model struggling on that sample). **The instrument for that is in** (2026-07-26, below in `DONE.md`): the eval line and every per-sample report record now carry `planner_ms`, `narrator_ms` and a `phases` attribution, so **the next step is a measurement, not code** — re-run the `hints_off_no_advice` arm at `CHESSAPP_EVAL_RUNS=20 CHESSAPP_EVAL_MAX_RUNS=20` with a report path set, and compare `planner_ms` on the `no_progress` samples against the `completed` ones in the same run. Matching planner time with divergent narrator time is the narrator; both up is a hard sample. Only then decide about bounding the narrator's thinking budget, which is the standing candidate fix and has been deferred twice already — and which would be a loop change with a real eval gate behind it, unlike the instrument.
