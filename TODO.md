...
- [ ] **[S] Watch the unasked-mutation shape now that hints mode is gone** (what remains of the flips-hints-unasked item; the capability question it posed was answered 2026-09-01 by retiring the mode — decision in PCC task #314, record in `DONE.md`). The measured defect was two-part: the flip itself (dead — no `set_hints_mode` exists) and, once, a move played after it (`set_hints_mode(enabled=true) → make_move(move="Bc4")`, 1 failure / 2 flips / 20 samples — suggestive, not established). The general shape "a question turn mutates something the player owns" is still only pinned scenario-by-scenario (`advice_is_engine_backed` asserts no board mutation; nothing asserts no settings mutation on a question turn). Fold a settings-mutation assertion into the next eval-touching slice rather than spending a dedicated one.
...
- [ ] Hints: a hint arrives when asked for ("what should I play?", "give me a hint") and is the engine's own suggestion; nothing is volunteered unasked (the standing hints mode was retired 2026-09-01 — there is no toggle to test)
...
- [ ] Settings by speech/text: difficulty, verbosity, voice output all switchable mid-game and persist
...
- [ ] **Merge the remaining settings setters? — weaker case than it was** (old audit finding 6) — the roster shrank when hints mode retired (2026-09-01): `set_verbosity`/`set_voice_output` are one shape; `set_option(name, value)` would now save ~100 tok across two tools, at the cost of a stringly-typed call that's easier to mis-invoke. Decide with the evals, not in the abstract — and only if the token pressure ever actually shows.
