...
### In progress (2026-07-21) — per-tool docstring hardening, one tool at a time

New sub-thread of the sprint: go through the write tools one by one and declutter each tool's *description* (finding 5's prose half, not its schema half — the schema cut is shelved, see below). Order: `make_move` → `undo` → `new_game` → `resign` → reads → analysis. Work is **uncommitted on branch `chore/clean-tool-schema-noise`**.

- **`make_move` — done, not committed.** Docstring cut ~200→~95 tok ([tools.py](backend/src/chessapp/tools.py), `make_move`), golden resynced ([tool_definitions_golden.json](backend/tests/tool_definitions_golden.json), clean one-line diff). Offline suite green (96 passed, ruff clean); the four `make_move` guard tests still pin the load-bearing anchors (`legal_moves`/`never invent`/`queen's bishop pawn` example/`once per player turn`/`announc`). **Eval gate:** its two relevant tripwires **`plain_move` + `undo_and_replace` pass**; the `long_capture` `live_like`/`poisoned` failures in the run are the **pre-existing #136 RED tracked below** (line begins "Also owns curing the base-prompt-shrink regression"), *not* caused by this cut. Full run: 20 passed, 1 xfailed, 2 failed (both the known `long_capture` params).
- **Finding to fold into finding 5 before the `undo`/`resign` passes:** the one line cut here as "redundant with the result payload" — *"the engine decides legality: the result says legal or illegal"* — is the most plausible **anti-self-poisoning anchor**, not dead weight: under a transcript seeded with stale "Illegal move." turns it re-asserts legality is decided *fresh each call*. Restoring it was tried and reverted (we're not chasing `long_capture`), but the lesson stands: **result-restating prose can be load-bearing specifically under self-poisoning.** `resign`/`new_game` carry the same shape ("when it returns ok, the game really did …") — do not cut it blindly.
- **Next:** decide whether to commit `make_move` on its own (tripwires pass; `long_capture` is already RED on `main`, so this doesn't move that gate) or bundle the write-tool passes into one PR.
