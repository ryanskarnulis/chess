...
3. **[S] Audit the remaining tools for the same bug class** — anything whose correct behavior is knowable from deterministic state but is currently left to the model's judgment (as `undo`'s ply count was). `resign`'s default color is done (it now defaults to the player, not the side to move), and so is `analyze_last_move`'s (it now defaults to the player's own last move); `new_game` is item 6 below. Anything left?
