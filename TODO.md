...
5. **[M] `resign` gets faked: the agent claims an op it never called** — `long_resign[poisoned]` regressed to **2/5** on 2026-07-13 (3/5 on a clean `main` re-run, so the path is a coin-flip, not the recorded 5/5): *"I'm calling that. Game over, bro."* — zero tool calls, board still live. This is trace-review finding 6, the **most dangerous class**: the player is *told* the thing happened. A prompt rule is the wrong instrument (the destructive-gate slice already showed a 12B follows one ~half the time) — look for a structural answer, e.g. the pipeline refusing to emit commentary that claims a destructive outcome no tool produced. The eval is a hard assert and currently red.
6. **[S] `analyze_last_move` can't mean "my" move** (trace finding 1, `my_mistake_is_mine` 0/5, xfailed) — no args, always the last ply, which on the player's turn is always the engine's reply. Default from `session.player_color`; allow an override.
7. **[S] `new_game` can't assign a side** (trace finding 2, `play_as_black` 0/5, xfailed) — no args, so "let's play chess as black" is impossible, and that is the **exact intent string in the advertised conductor deep link**, so the handoff is broken end to end. `GameSession.new_game(player_color=…)` already supports it.
8. **[M] Show tool calls live in the UI, like "thinking…"** — the player should see the agent's tools as it calls them (`undo` → `make_move`), not just the commentary after. Needs the tool trajectory streamed or polled during a turn rather than returned only in the final `/api/command` response; the trace record is already the right shape for it.
