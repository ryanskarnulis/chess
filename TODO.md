...
- [ ] **[S] A repeat-stop turn narrates 2–3× slower than an ordinary one** (spin-off, measured by the repeat-stop slice — cause **not** established). On the two 20-sample `hints_off_no_advice` arms, `no_progress` turns took a median 40.5 s (no planner note) and ≈26.7 s (with the loop's note) against 9.9–11.5 s for `completed` samples in the same runs, at the same model-call count — so it is the narrator's own round trip, thinking ON. Giving the narrator a note helped and did not explain it. The other reading is that the repeat and the long narration share a cause (the model struggling on that sample) rather than one causing the other, which the harness cannot currently tell apart: `model_ms` is a per-turn sum, so **the first step is per-call latencies in the eval line** (`AgentResponse.model_latencies_ms` already carries them and the trace already records them — the harness just doesn't print them). Worth doing before any attempt to bound the narrator's thinking budget, which is the standing candidate fix and has been deferred twice already.
