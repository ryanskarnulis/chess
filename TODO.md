...
- [ ] **[M] Bound the narrator's thinking budget — the measurement is in, and it says the cap is aimed right but will do less than it looks.** Read 2026-07-27 on a clean 20-sample `hints_off_no_advice` arm (`docs/agent-evals.md`): a repeat-stop narration writes **2.58× the tokens** at a **flat rate** (0.98×, p = 0.30), and `narrator_ms` tracks `narrator_out` at **r² = 0.998**. Serving path excluded, prefill excluded (`narrator_in` 1,001 vs 993). So the mechanism is wordiness and the standing fix is aimed at it. **The catch is the number.** Legitimate `completed`-and-passing narrations reach **1,259** tokens and the repeat tail is **521–2,633** — the distributions overlap almost entirely, so a cap safe for ordinary narration (~1,300) clips only 3/20 samples and a cap that catches the median tail (~600) truncates real commentary. Decide whether a ceiling on the runaway is worth a loop change at all before writing one; if yes, it is a `src/` change and owes a full gate, with `long_capture` staying green.
