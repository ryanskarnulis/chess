...
- [ ] **[S] `hints_off_no_advice` is red on `main` — find out why** (regression, measured 2026-07-25). Slice 1 recorded 5/5; it does not reproduce. Isolated 5-run rates on clean `main` (aeb1aea): 2/5, 0/5, 2/5 — and the structured-errors branch measures the same, so nothing in Sprint 3 slice 3 caused it (full A/B table in `docs/agent-evals.md`). Every failure has one shape: the planner calls `analyze_last_move` as well as `evaluate_position`, `api._advice_evidence` reads that as "analysis the player asked for", and the advice guard stands down. Two candidate fixes, and the measurement should pick between them rather than the other way round: **tighten the evidence test** (a `get_best_moves`/`analyze_last_move` result should only license naming the moves *it* reported, not any legal move), or find what changed since slice 1 — leading suspect #155's per-turn `settings` block, which landed after the 5/5 was recorded and whose merge note mentions no eval run. Overlaps the honesty-guard slice below; do this first, since it decides whether that slice inherits a working guard.
