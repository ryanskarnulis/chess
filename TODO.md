...
- [ ] **[S] `llama_brain` swallows the provider's message** (spin-off, found in the eval-statistics slice). `llama_brain.py:202` and `:276` are bare `except ProviderError:` and discard the message, so a crashed socket and an HTTP 400 are indistinguishable to the harness — which matters now that the harness *retries* one and must not retry the other five times before aborting. Carry the reason through to `stop_reason`'s neighbourhood (a field, not a prose blob) so a deterministic failure can be told from a dead server.
- [ ] **[S] A declined question should not burn four planner iterations** (spin-off, found in the eval-statistics slice). `hints_off_no_advice` reached `max_iterations` on three of four "passes" in one recorded run: the planner spends its whole budget on an ask whose right answer is "no, hints are off". That is a real loop defect — a budget stop reaches no narrator, so the player gets a canned line — and it is **not** a floor to slide. The eval now counts those samples as `INCONCLUSIVE` rather than passes, so the scenario's rate will read lower until this is fixed.
