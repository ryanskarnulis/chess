...
- [ ] **[M] Full planner/narrator split** (audit 15 — elevated from Sprint 4 to P0, Ryan's call 2026-07-25). Two model phases with different prompts and sampling. The **planner** is still the bounded multi-step tool loop #124 built — that win is kept; what leaves it is the *voice* — run on a compact, persona-free contract prompt at lower temperature, so tool selection stops competing with Glitch's tone for a 12B's attention (the harness audit's core diagnosis). The **narrator** speaks as Glitch from the turn's verified results on the personality-rich prompt, offered no tools — the generalization of `narrate()`, which already does exactly this for the fast path, and the natural voice of the coordinator's observe/close beats (next item). Absorbs the old per-phase sampling experiment: planner temperature is measured inside this slice, tool-call accuracy recorded before any default changes. Eval-gated like every prompt/loop change, and mind the two standing constraints above: persona leaving the planner prompt is exactly the kind of reshuffle that moved `long_capture` before (re-run it), and the compaction must not touch schema structure.
- [ ] **[M] Split the move flow: Glitch observes between the player's move and the engine's reply** (audit 2, 5). `make_move` (the tool) and `/api/game/move` both become "apply player move" only; the coordinator then hands the narrator structured facts (move, capture, check/mate/draw status, new FEN) for a short reaction — streamed or run while Stockfish calculates — then applies the engine reply and offers the close. Observation beats are skippable and bounded; low verbosity skips them entirely.
